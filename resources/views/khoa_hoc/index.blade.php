...
<div class="space-y-6">
    ...
    @if(session('status')) <x-quan_tri.thong_bao :session="session()" /> @endif
    @if(session('error')) <x-quan_tri.thong_bao :session="session()" /> @endif
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition">
            <div class="flex justify-between items-start">
                ...
                <x-quan_tri.huy_hieu :type="'info'" :text="$course->statusLabel()" />
            </div>
