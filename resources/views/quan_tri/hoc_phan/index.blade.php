...
@section('content')
<div class="space-y-6">
    <x-quan_tri.tieu_de_trang title="Quản lý module" subtitle="Tổng hợp module theo từng lớp học, số buổi và trạng thái hiển thị.">
        <x-slot name="actions">
    ...
    <div class="grid gap-4 md:grid-cols-3">
        <x-quan_tri.the_thong_ke label="Lớp học có module" :value="$summary['course_count']" icon="fas fa-people-group" color="cyan" />
        <x-quan_tri.the_thong_ke label="Tổng module" :value="$summary['module_count']" icon="fas fa-cubes-stacked" color="emerald" />
        <x-quan_tri.the_thong_ke label="Module đang hiển thị" :value="$summary['published_module_count']" icon="fas fa-eye" color="amber" />
    </div>

    <x-quan_tri.thanh_loc :route="route('admin.modules.index')" searchPlaceholder="Tên lớp học, khóa học hoặc nhóm học..." />
