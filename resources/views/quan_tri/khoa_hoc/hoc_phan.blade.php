...
<div class="space-y-6">
    <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
            ...
            <p class="mt-2 text-sm text-slate-600">Lớp học: <strong>{{ $course->title }}</strong></p>
        </div>
        ...
        <aside class="space-y-6">
            <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <form method="post" action="{{ route('admin.courses.modules.create', $course) }}" class="mt-5 space-y-4">
                    @csrf
                    @include('quan_tri.khoa_hoc._mau_hoc_phan', ['module' => null, 'defaultPosition' => ((int) $modules->max('position')) + 1])
