    ...
    <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
            ...
            <div class="mt-3 flex flex-wrap items-center gap-3 text-sm text-slate-600">
                <span>{{ $course->subject?->name ?? 'Chưa gắn khóa học công khai' }}</span>
    ...
    <div class="grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,1fr)]">
        <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <form
                ...
                data-schedule-preview-url="{{ route('admin.courses.schedule-preview', $course) }}"
            ...
            >
                        ...
                        <div class="sm:w-96">
                            ...
                            <select id="schedule-template" class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm text-slate-900 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-100">
                                ...
                                @foreach ($scheduleTemplates as $key => $template)
                                    <option
                                        value="{{ $key }}"
                                        data-days="{{ implode(',', $template['days']) }}"
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="date" name="start_date" value="{{ $selectedStartDate }}" class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm text-slate-900 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-100">
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-medium text-slate-700">Ngày kết thúc</label>
                        <input type="date" name="end_date" value="{{ $selectedEndDate }}" class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm text-slate-900 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-100">
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-medium text-slate-700">Giờ bắt đầu</label>
                        <input type="time" name="start_time" value="{{ $selectedStartTime }}" class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm text-slate-900 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-100">
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-medium text-slate-700">Giờ kết thúc</label>
                        <input type="time" name="end_time" value="{{ $selectedEndTime }}" class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm text-slate-900 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-100">
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600">
                            ...
                            <span id="schedule-preview" class="mt-2 block">{{ $course->formattedSchedule() }}</span>
                        </div>
                        @error('meeting_days')
