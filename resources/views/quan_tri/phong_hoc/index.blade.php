...
@section('content')
<div class="space-y-6">
    <x-quan_tri.tieu_de_trang title="Quản lý phòng học" subtitle="Quản lý danh sách phòng, sức chứa và tình trạng sẵn sàng cho việc mở lớp theo slot.">
        <x-slot name="actions">
    ...
    <div class="grid gap-4 md:grid-cols-4">
        <x-quan_tri.the_thong_ke label="Tổng phòng" :value="$summary['total']" icon="fas fa-door-open" color="cyan" />
        <x-quan_tri.the_thong_ke label="Đang hoạt động" :value="$summary['active']" icon="fas fa-circle-check" color="emerald" />
        <x-quan_tri.the_thong_ke label="Bảo trì" :value="$summary['maintenance']" icon="fas fa-screwdriver-wrench" color="amber" />
                    ...
                    @forelse ($rooms as $room)
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @endphp
                                <x-quan_tri.huy_hieu :type="$type" :text="$room->statusLabel()" />
                            </td>
