...
@section('content')
<div class="space-y-6">
    <x-quan_tri.tieu_de_trang title="Quản lý khung giờ học" subtitle="Theo dõi cấu hình mở slot, giảng viên, phòng học và số lượng nguyện vọng trên từng khung giờ.">
        <x-slot name="actions">
    ...
    <div class="grid gap-4 md:grid-cols-4">
        <x-quan_tri.the_thong_ke label="Đã cấu hình" :value="$summary['configured']" icon="fas fa-clock" color="cyan" />
        <x-quan_tri.the_thong_ke label="Đang mở đăng ký" :value="$summary['open']" icon="fas fa-door-open" color="emerald" />
        <x-quan_tri.the_thong_ke label="Đủ điều kiện mở lớp" :value="$summary['ready']" icon="fas fa-circle-check" color="amber" />
        <x-quan_tri.the_thong_ke label="Đã mở lớp" :value="$summary['opened']" icon="fas fa-graduation-cap" color="slate" />
    </div>

    <x-quan_tri.thanh_loc :route="route('admin.course-time-slots.index')" searchPlaceholder="Khóa học, giảng viên, phòng học..." :statuses="$statuses">
                    ...
                    @forelse ($timeSlots as $timeSlot)
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @endphp
                                <x-quan_tri.huy_hieu :type="$type" :text="$timeSlot->statusLabel()" />
                            </td>
