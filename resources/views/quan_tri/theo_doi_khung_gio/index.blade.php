...
@section('content')
<div class="space-y-6">
    <x-quan_tri.tieu_de_trang title="Theo dõi theo slot" subtitle="Nhìn nhanh mức độ lấp đầy, số nguyện vọng chờ và tiến độ mở lớp trên từng khung giờ học.">
        <x-slot name="actions">
    ...
    <div class="grid gap-4 md:grid-cols-4">
        <x-quan_tri.the_thong_ke label="Slot đang mở" :value="$summary['open_slots']" icon="fas fa-door-open" color="cyan" />
        <x-quan_tri.the_thong_ke label="Slot đủ điều kiện" :value="$summary['ready_slots']" icon="fas fa-circle-check" color="emerald" />
        <x-quan_tri.the_thong_ke label="Tổng lựa chọn" :value="$summary['choices']" icon="fas fa-list-check" color="amber" />
        <x-quan_tri.the_thong_ke label="Nhu cầu cao nhất" :value="$summary['top_demand']" icon="fas fa-chart-line" color="slate" />
    </div>

    <x-quan_tri.thanh_loc :route="route('admin.slot-tracking.index')" :statuses="$statuses">
