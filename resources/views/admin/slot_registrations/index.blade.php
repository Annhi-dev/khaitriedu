...
@section('content')
<div class="space-y-6">
    <x-admin.page-header title="Quản lý nguyện vọng slot" subtitle="Theo dõi nhu cầu học theo khung giờ để admin duyệt và gom lớp chính xác hơn.">
        <x-slot name="actions">
    ...
    <div class="grid gap-4 md:grid-cols-4">
        <x-admin.stat-card label="Tổng nguyện vọng" :value="$summary['total']" icon="fas fa-list-check" color="cyan" />
        <x-admin.stat-card label="Chờ xử lý" :value="$summary['pending']" icon="fas fa-hourglass-half" color="amber" />
        <x-admin.stat-card label="Đã xếp lớp" :value="$summary['scheduled']" icon="fas fa-graduation-cap" color="emerald" />
        <x-admin.stat-card label="Cần chọn lại" :value="$summary['needs_reselect']" icon="fas fa-rotate" color="rose" />
    </div>

    <x-admin.filter-bar :route="route('admin.slot-registrations.index')" searchPlaceholder="Học viên, khóa học hoặc ghi chú..." :statuses="$statuses">
                    ...
                    @forelse ($registrations as $registration)
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @endphp
                                <x-admin.badge :type="$type" :text="$registration->statusLabel()" />
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-700">
