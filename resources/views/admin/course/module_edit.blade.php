...
<div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
            ...
            <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Điều chỉnh nội dung module trong lớp <strong>{{ $course->title }}</strong> để giữ lộ trình học rõ ràng và đúng thứ tự.</p>
        </div>
    ...
    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="post" action="{{ route('admin.courses.modules.update', [$course, $module]) }}" class="space-y-4">
            @csrf
            @include('admin.course._module_form', ['module' => $module])
