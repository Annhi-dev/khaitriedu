...
<div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <a href="{{ route('admin.teachers.create') }}" class="inline-flex items-center justify-center rounded-2xl bg-cyan-600 px-5 py-3 text-sm font-semibold text-white hover:bg-cyan-700">Thêm giảng viên mới</a>
    </div>

    <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <form method="get" action="{{ route('admin.teachers.index') }}" class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_240px_auto]">
            <div>
            ...
            <div class="flex items-end gap-3">
                ...
                <a href="{{ route('admin.teachers.index') }}" class="inline-flex items-center justify-center rounded-2xl border border-slate-300 px-4 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50">Xóa lọc</a>
            </div>
        </form>
    </div>
    ...
    <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div class="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-sm text-slate-500">Tổng cộng {{ $teachers->total() }} giảng viên phù hợp với bộ lọc hiện tại.</p>
            </div>
        </div>
                        ...
                        @foreach ($teachers as $teacher)
                            ...
                            <tr class="align-top">
                                <td class="px-5 py-4">
                                    ...
                                    <div class="mt-1 text-xs uppercase tracking-wide text-slate-500">{{ $teacher->username }}</div>
                                </td>
                                <td class="px-5 py-4 text-slate-600">
                                    <div>{{ $teacher->email }}</div>
                                    <div class="mt-1 text-xs text-slate-500">{{ $teacher->phone ?: 'Chưa cập nhật số điện thoại' }}</div>
                                </td>
                                <td class="px-5 py-4">
                                    <span class="inline-flex rounded-full border px-3 py-1 text-xs font-semibold {{ $statusClasses }}">{{ $teacher->statusLabel() }}</span>
                                </td>
                                <td class="px-5 py-4 text-slate-600">{{ $teacher->taught_courses_count }}</td>
                                <td class="px-5 py-4 text-slate-600">{{ $teacher->schedule_change_requests_count }}</td>
                                <td class="px-5 py-4">
                                    <div class="flex flex-wrap items-center justify-end gap-2">
                                        <a href="{{ route('admin.teachers.show', $teacher) }}" class="inline-flex items-center justify-center rounded-xl border border-slate-300 px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50">Xem</a>
                                        <a href="{{ route('admin.teachers.edit', $teacher) }}" class="inline-flex items-center justify-center rounded-xl border border-cyan-200 px-3 py-2 text-xs font-medium text-cyan-700 hover:bg-cyan-50">Sửa</a>
                                        @if ($teacher->isLocked())
