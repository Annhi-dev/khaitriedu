...
<div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="grid gap-6 p-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)] lg:p-8">
            ...
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                @foreach ($summaryCards as $card)
                    <x-admin.stat-card :label="$card['label']" :value="$card['value']" :icon="$card['icon']" color="cyan" :note="$card['note']" />
                @endforeach
            </div>
        </div>
    </section>
    ...
    <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($teachers as $teacher)
                        <tr class="group transition hover:bg-slate-50/80">
                            ...
                            <td class="px-6 py-4">
                                <x-admin.badge :type="match ($teacher->status) {
        ...
        <div class="border-t border-slate-100 p-6">
            <x-admin.pagination :paginator="$teachers" label="giảng viên" />
        </div>
    </section>
</div>
@endsection
