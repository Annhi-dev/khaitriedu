...
<div class="space-y-6">
    ...
    <div class="grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(360px,0.9fr)]">
        ...
        <aside class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p class="mt-2 text-sm leading-6 text-slate-600">Duyệt đăng ký để chuyển sang bước chờ xếp lớp, hoặc xếp lớp ngay nếu đã có lớp phù hợp. Khi để trống giảng viên và lịch, hệ thống sẽ ưu tiên lấy từ lớp nội bộ đã chọn.</p>

            @if (in_array($normalizedStatus, [\App\Models\Enrollment::STATUS_PENDING, \App\Models\Enrollment::STATUS_APPROVED], true))
                <a href="{{ route('admin.schedules.enrollments.show', $enrollment) }}" class="mt-4 inline-flex items-center justify-center rounded-2xl border border-cyan-200 bg-cyan-50 px-4 py-2.5 text-sm font-semibold text-cyan-700 hover:bg-cyan-100">Mở màn xếp lịch phase 9</a>
            @endif
