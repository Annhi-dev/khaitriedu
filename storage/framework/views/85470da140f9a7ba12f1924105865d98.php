...
<?php $__env->startSection('eyebrow', 'Kết quả học tập'); ?>
    ...
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div class="mt-6 grid gap-3 sm:grid-cols-3">
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <p class="mt-2 text-2xl font-semibold text-slate-900"><?php echo e($gradeList->count()); ?></p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">Có điểm số</p>
                <p class="mt-2 text-2xl font-semibold text-slate-900"><?php echo e($gradeList->whereNotNull('score')->count()); ?></p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">Chưa có điểm</p>
                <p class="mt-2 text-2xl font-semibold text-slate-900"><?php echo e($gradeList->whereNull('score')->count()); ?></p>
            </div>
        </div>
    </section>
            ...
            <article class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div class="min-w-0 flex-1">
                        <h3 class="text-lg font-semibold text-slate-900"><?php echo e($courseTitle); ?></h3>
                        <div class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">Module</p>
                                <p class="mt-2 text-sm font-medium text-slate-900"><?php echo e($grade->module->title ?? 'Tổng kết'); ?></p>
                            </div>
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">Điểm số</p>
                                <p class="mt-2 text-sm font-medium text-slate-900"><?php echo e($grade->score ?? 'Chưa có'); ?></p>
                            </div>
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">Điểm chữ</p>
                                <p class="mt-2 text-sm font-medium text-slate-900"><?php echo e($grade->grade ?? 'Chưa có'); ?></p>
                            </div>
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">Cập nhật</p>
                                <p class="mt-2 text-sm font-medium text-slate-900"><?php echo e($updatedAt ?? 'Chưa rõ'); ?></p>
                            </div>
                        </div>

                        <div class="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Phản hồi</p>
                            <p class="mt-2 text-sm leading-6 text-slate-700"><?php echo e($grade->feedback ?: 'Không có phản hồi'); ?></p>
                        </div>
                    </div>
                </div>
            </article>
...
<?php echo $__env->make('bo_cuc.hoc_vien', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\XXamp\htdocs\khaitriedu-main\resources\views/hoc_vien/diem_so.blade.php ENDPATH**/ ?>