...
<div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <a href="<?php echo e(route('admin.categories.create-page')); ?>" class="inline-flex items-center justify-center rounded-2xl bg-cyan-600 px-5 py-3 text-sm font-semibold text-white hover:bg-cyan-700">Tạo nhóm học mới</a>
    </div>

    <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <form method="get" action="<?php echo e(route('admin.categories')); ?>" class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_220px_auto]">
            <div>
                <label class="mb-2 block text-sm font-medium text-slate-700">Tìm kiếm</label>
                <input name="search" value="<?php echo e($filters['search'] ?? ''); ?>" placeholder="Tên, slug, mô tả, chương trình, cấp độ" class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm text-slate-900 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-100" />
            </div>
            <div>
                ...
                <select name="status" class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm text-slate-900 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-100">
                    ...
                    <option value="<?php echo e(\App\Models\Category::STATUS_ACTIVE); ?>" <?php if(($filters['status'] ?? '') === \App\Models\Category::STATUS_ACTIVE): echo 'selected'; endif; ?>>Hoạt động</option>
                    <option value="<?php echo e(\App\Models\Category::STATUS_INACTIVE); ?>" <?php if(($filters['status'] ?? '') === \App\Models\Category::STATUS_INACTIVE): echo 'selected'; endif; ?>>Ngừng hoạt động</option>
                </select>
            </div>
            <div class="flex items-end gap-3">
                ...
                <a href="<?php echo e(route('admin.categories')); ?>" class="inline-flex items-center justify-center rounded-2xl border border-slate-300 px-4 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50">Xóa lọc</a>
            </div>
        </form>
    </div>
    ...
    <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div class="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-sm text-slate-500">Tổng cộng <?php echo e($categories->total()); ?> nhóm học phù hợp với bộ lọc hiện tại.</p>
            </div>
        </div>
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <tr class="align-top">
                                <td class="px-5 py-4">
                                    <div class="font-semibold text-slate-900"><?php echo e($category->name); ?></div>
                                    <div class="mt-1 text-xs text-slate-500">/<?php echo e($category->slug); ?></div>
                                    <p class="mt-3 max-w-xl text-sm leading-6 text-slate-600"><?php echo e($category->description ?: 'Chưa có mô tả cho nhóm học này.'); ?></p>
                                </td>
                                <td class="px-5 py-4 text-slate-600">
                                    <div><?php echo e($category->program ?: 'Chưa cấu hình chương trình'); ?></div>
                                    <div class="mt-1 text-xs text-slate-500">Cấp độ: <?php echo e($category->level ?: 'Chưa xác định'); ?></div>
                                    <div class="mt-1 text-xs text-slate-500">Thứ tự hiển thị: <?php echo e($category->order); ?></div>
                                </td>
                                <td class="px-5 py-4">
                                ...
                                <td class="px-5 py-4 text-slate-600">
                                    <div><?php echo e($category->subjects_count); ?> khóa học public</div>
                                    <div class="mt-1 text-xs text-slate-500"><?php echo e($category->courses_count); ?> lớp học nội bộ</div>
                                </td>
                                <td class="px-5 py-4">
                                    <div class="flex flex-wrap items-center justify-end gap-2">
                                        <a href="<?php echo e(route('admin.categories.show', $category)); ?>" class="inline-flex items-center justify-center rounded-xl border border-slate-300 px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50">Xem</a>
                                        <a href="<?php echo e(route('admin.categories.edit', $category)); ?>" class="inline-flex items-center justify-center rounded-xl border border-cyan-200 px-3 py-2 text-xs font-medium text-cyan-700 hover:bg-cyan-50">Sửa</a>
