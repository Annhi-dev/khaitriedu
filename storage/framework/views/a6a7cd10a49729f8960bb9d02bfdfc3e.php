...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'Khu vực học viên'); ?> - KhaiTriEdu</title>
...
<div x-data="{ sidebarOpen: false }" class="relative min-h-screen">
    <div id="student-shell" class="lg:grid lg:grid-cols-[280px_minmax(0,1fr)] transition-all duration-300">
        ...
        <aside id="student-sidebar" class="fixed inset-y-0 left-0 z-50 w-80 -translate-x-full overflow-y-auto border-r border-slate-200 bg-white transition duration-300 lg:static lg:w-auto lg:translate-x-0"
            ...
            <div class="px-4 py-5">
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <p class="mt-3 text-lg font-semibold text-slate-900"><?php echo e($studentUser?->name ?? 'Học viên'); ?></p>
                    <p class="mt-1 text-sm text-slate-500"><?php echo e($studentUser?->email ?? 'student@khaitriedu.local'); ?></p>
                </div>
            </div>
            ...
            <nav class="px-4 pb-6 space-y-5">
                <?php $__currentLoopData = $navSections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $section): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div>
                        <p class="px-3 text-[10px] font-semibold uppercase tracking-[0.22em] text-slate-400"><?php echo e($section['label']); ?></p>
                        <div class="mt-2 space-y-1">
                            <?php $__currentLoopData = $section['items']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <a href="<?php echo e(route($item['route'])); ?>"
                                    class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition <?php echo e($item['active'] ? 'bg-cyan-50 text-cyan-700 ring-1 ring-cyan-100' : 'text-slate-600 hover:bg-slate-50 hover:text-cyan-700'); ?>">
                                    <i class="fas <?php echo e($item['icon']); ?> w-5 text-center text-sm <?php echo e($item['active'] ? 'text-cyan-600' : 'text-slate-400'); ?>"></i>
                ...
                <div class="space-y-1">
                    <a href="<?php echo e(route('home')); ?>" class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-cyan-700">
        ...
        <main class="min-w-0">
            <header class="sticky top-0 z-30 border-b border-slate-200 bg-white/90 backdrop-blur">
                <div class="flex items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
                    <div class="flex min-w-0 items-center gap-3">
                        ...
                        <div class="min-w-0">
                            <?php if($pageEyebrow): ?>
                                <p class="text-xs font-semibold uppercase tracking-[0.22em] text-cyan-700"><?php echo e($pageEyebrow); ?></p>
                            <?php endif; ?>
                            <h1 class="mt-1 truncate text-xl font-semibold text-slate-900"><?php echo $__env->yieldContent('title', 'Khu vực học viên'); ?></h1>
                        </div>
                    </div>

                    <div class="flex items-center gap-3">
                        <div x-data="{ notificationsOpen: false }" class="relative" data-notification-poller data-notification-poller-url="<?php echo e(route('student.notifications.poll')); ?>">
                            <button type="button" @click="notificationsOpen = !notificationsOpen" class="relative inline-flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 text-slate-700 transition hover:border-cyan-200 hover:bg-cyan-50 hover:text-cyan-700" title="Thông báo">
                                ...
                                <?php if($studentUnreadNotifications > 0): ?>
                                    <span data-notification-badge class="absolute -right-1 -top-1 min-w-[18px] rounded-full bg-red-500 px-1.5 py-0.5 text-center text-[10px] font-semibold leading-none text-white">
                                        <?php echo e($studentUnreadNotifications > 99 ? '99+' : $studentUnreadNotifications); ?>
                            ...
                            <div x-show="notificationsOpen" x-cloak @click.away="notificationsOpen = false" class="absolute right-0 z-50 mt-3 w-96 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
                                <div class="border-b border-slate-100 px-5 py-4">
                                    ...
                                    <p class="mt-1 text-xs text-slate-500"><span data-notification-unread-count><?php echo e($studentUnreadNotifications); ?></span> chưa đọc</p>
                                </div>

                                <div class="max-h-96 overflow-y-auto" data-notification-list>
                                    <?php $__empty_1 = true; $__currentLoopData = $studentNotificationItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $notification): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                        <a href="<?php echo e(route('student.notifications.show', $notification)); ?>" class="block border-b border-slate-100 px-5 py-4 transition hover:bg-slate-50">
                                ...
                                <a href="<?php echo e(route('student.notifications.index')); ?>" class="block border-t border-slate-100 px-5 py-3 text-sm font-semibold text-cyan-700 transition hover:bg-cyan-50">
                                    Mở hộp thông báo
                                </a>
                            </div>
                        </div>
                        ...
                        <a href="<?php echo e(route('home')); ?>" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:border-cyan-200 hover:bg-cyan-50 hover:text-cyan-700">
                            <i class="fas fa-house"></i>
                            <span class="hidden sm:inline">Trang chủ</span>
                        </a>

                        <a href="<?php echo e(route('student.profile.show')); ?>" class="hidden items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-600 md:flex">
                            <?php if($studentUser?->avatarUrl()): ?>
                                <img src="<?php echo e($studentUser->avatarUrl()); ?>" alt="Avatar" class="h-9 w-9 rounded-full object-cover ring-2 ring-cyan-100">
                            <?php else: ?>
                                <span class="inline-flex h-9 w-9 items-center justify-center rounded-full bg-cyan-500/10 font-semibold text-cyan-700">
                                    <?php echo e(strtoupper(substr($studentUser?->name ?? 'H', 0, 1))); ?>

                                </span>
                            <?php endif; ?>
                            <div>
                                <p class="font-medium text-slate-800"><?php echo e($studentUser?->name ?? 'Học viên'); ?></p>
                                <p class="text-xs text-slate-500"><?php echo e($studentUser?->roleLabel() ?? 'Học viên'); ?></p>
                            </div>
                        </a>
                    </div>
                </div>
            ...
            <?php if(!request()->routeIs('student.dashboard')): ?>
                <div class="border-b border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-500 sm:px-6 lg:px-8">
                <a href="<?php echo e(route('student.dashboard')); ?>" class="hover:text-cyan-700">Tổng quan</a>
...
<?php /**PATH D:\XXamp\htdocs\khaitriedu-main\resources\views/bo_cuc/hoc_vien.blade.php ENDPATH**/ ?>