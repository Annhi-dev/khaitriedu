...
$supportEmails = array_values(array_filter(array_map(
    'trim',
    explode(',', (string) env('SITE_CONTACT_EMAILS', 'support@khaitriedu.vn,info@khaitriedu.vn'))
)));
$contactRecipient = trim((string) env('SITE_CONTACT_RECIPIENT', (string) env('MAIL_FROM_ADDRESS', 'no-reply@khaitriedu.vn')));

if ($contactRecipient === '') {
    $contactRecipient = $supportEmails[0] ?? 'no-reply@khaitriedu.vn';
}

if ($supportEmails === []) {
    $supportEmails = [$contactRecipient];
}
...
return [
    ...
    'legal' => [
        'terms_updated_at' => env('SITE_TERMS_UPDATED_AT', '2026-03-01'),
        'privacy_updated_at' => env('SITE_PRIVACY_UPDATED_AT', '2026-03-01'),
        'terms_email' => env('SITE_LEGAL_EMAIL', $contactRecipient),
        'privacy_email' => env('SITE_PRIVACY_EMAIL', $contactRecipient),
    ],
];
